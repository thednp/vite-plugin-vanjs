...
*/
import { ChildDom, PropsWithKnownKeys as PropsWithKnownKeys$1, State, StateView } from "vanjs-core";
import { Properties, PropertiesHyphen, SvgProperties, SvgPropertiesHyphen } from "csstype";
import { Element as Element$1, TagFunc } from "mini-van-plate/van-plate";
...
type PropValue = Omit<Primitive, bigint> | null;
type PropsWithKnownKeys<T> = Partial<{ [K in keyof T]: StateMaybe<T[K]> }>;
type EventNameMap = keyof HTMLElementEventMap;
type StyleValue = DOMElementsMap["main"]["style"];
type DOMNodeAttributes<K extends TagNames> = DOMElementsMap[K] & {
  children?: MaybeChildNode[];
  style?: StyleValue;
  class?: StateMaybe<string>; // ref?: <T extends Element = OutputElement<K>>(el: T) => void;
  [key: `data-${string}`]: FunctionMaybe<string | boolean | number>;
  [key: `aria-${string}`]: FunctionMaybe<string>;
};
type OutputElement<K extends TagNames> = K extends keyof MathMLElementTagNameMap ? MathMLElement : DOMTagNameMap[K];
type ComponentProps<K extends keyof DOMElementsMap> = DOMElementsMap[K];
type Component<K extends TagNames, O extends ComponentProps<K> | undefined = undefined> = (props?: (O extends object ? ComponentProps<K> & Partial<O> : ComponentProps<K>) & {
...
type PotentialEventTarget = typeof globalThis | Window | Document | Element | MediaQueryList;
type PotentialEventObject = Event | AnimationEvent | ClipboardEvent | CompositionEvent | DragEvent | FocusEvent | SubmitEvent | KeyboardEvent | MouseEvent | PointerEvent | TouchEvent | TransitionEvent | UIEvent | WheelEvent;
//#endregion
...
type TagNames = keyof DOMElementsMap & (keyof HTMLElementTagNameMap | keyof SVGElementTagNameMap);
//#endregion
...
type ComponentFn = ((node?: unknown) => Element$1) | (() => ElementUnion) | (<T extends TagNames = "main", C = DOMNodeAttributes<T>>(props?: C) => OutputElement<T>);
type RouteEntry = {
  path: string;
...
type TagProps = SupportedTags | PropsWithKnownKeys$1<SupportedTags>;
...
type VanJSPluginOptions = {
  routesDir?: string;
  extensions?: string[];
};
type VanJSPlugin = (config?: VanJSPluginOptions) => Plugin<VanJSPluginOptions>;
//#endregion
//#region src/plugin/index.d.ts
declare const VitePluginVanJS: VanJSPlugin;
//#endregion
export { Accessor, AllHeadTags, type CSSProperties, ChildAccessor, ChildArray, Component, ComponentFn, ComponentModule, ComponentProps, DOMElement, type DOMElementsMap, DOMNodeAttributes, DynamicModule, type ElementUnion, type EventHandler, type EventHandlerUnion, EventNameMap, FNObject, FileRouteConfig, Fragment, FunctionMaybe, HeadTags, ImportFn, LayoutFile, LazyComponent, MaybeChildNode, ObserverFn, OutputElement, PageFile, PluginConfig, PotentialEventObject, PotentialEventTarget, PotentialProps, Primitive, PropValue, PropsWithKnownKeys, RouteEntry, RouteFile, RouteProps, StateEntry, StateMaybe, StoreObject, StoreValue, SupportedTags, type TagNames, TagProps, VanJSPlugin, VanJSPluginOptions, VitePluginVanJS as default };
